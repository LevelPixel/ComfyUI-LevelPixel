...
class LoraTagLoader:
    ...
    def load_lora(self, model, clip, text):
        ...
        for f in founds:
            ...
            for lora_file in lora_files:
                if Path(lora_file).name.startswith(name) or lora_file.startswith(name):
                    ...
                    break
            if lora_name == None:
                log[0] = log[0] + f"NOT found LoRA '{name}' \n"
        ...
        return {"ui": {"log": log}, "result": (model_lora, clip_lora, plain_prompt)}

NODE_CLASS_MAPPINGS = {
    "LoraTagLoader|LP": LoraTagLoader,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "LoraTagLoader|LP": "Load LoRA Tag [LP]",
}