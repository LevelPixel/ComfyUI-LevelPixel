...
def log(message, type=None, always=False):
    ...
    print(f"(LevelPixel:{name}) {message}")
...
def update_node_status(client_id, node, text, progress=None):
    ...
    PromptServer.instance.send_sync("levelpixel/update_status", {
        "node": node,
        "progress": progress,
        "text": text
    }, client_id)
...
async def update_node_status_async(client_id, node, text, progress=None):
    ...
    await PromptServer.instance.send("levelpixel/update_status", {
        "node": node,
        "progress": progress,
        "text": text
    }, client_id)
...
@PromptServer.instance.routes.get("/levelpixel/autotagger/tag")
